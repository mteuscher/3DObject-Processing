...
function start3DOC(input, output, file, centroid, centres, surface, objects, masked, statistics) {
	...
	run("3D OC Options - MT version", "volume surface nb_of_obj._voxels nb_of_surf._voxels integrated_density mean_gray_value std_dev_gray_value median_gray_value minimum_gray_value maximum_gray_value centroid mean_distance_to_surface std_dev_distance_to_surface median_distance_to_surface min_distance_to_surface max_distance_to_surface aspect-ratio_of_distance_to_surface centre_of_mass bounding_box show_masked_image_(redirection_requiered) dots_size=5 font_size=10 show_numbers white_numbers store_results_within_a_table_named_after_the_image_(macro_friendly) redirect_to=Substack-" + list[i]);
	run("3D Objects Counter - MT version", "threshold=128 slice=261 min.=150 max.=4000 exclude_objects_on_edges objects surfaces centroids centres_of_masses statistics summary");
